...
# Page Config
st.set_page_config(page_title="Olist BI Dashboard", layout="wide", initial_sidebar_state="expanded")
...
if engine:
    try:
        ...
        with col1:
            res = pd.read_sql("SELECT COUNT(order_id) as c FROM orders", engine)
            val = res['c'][0] if not res.empty else 0
            st.metric("Ordini Totali", f"{val:,}")
        
        with col2:
            res = pd.read_sql("SELECT SUM(payment_value) as s FROM order_payments", engine)
            val = res['s'][0] if not res.empty and res['s'][0] is not None else 0
            st.metric("Fatturato Lordo", f"R$ {val:,.2f}")
        
        with col3:
            res = pd.read_sql("SELECT AVG(review_score) as a FROM order_reviews", engine)
            val = res['a'][0] if not res.empty and res['a'][0] is not None else 0
            st.metric("Rating Medio", f"{val:.2f} / 5")
        
        with col4:
            res = pd.read_sql("SELECT COUNT(DISTINCT customer_unique_id) as c FROM customers", engine)
            val = res['c'][0] if not res.empty else 0
            st.metric("Clienti Unici", f"{val:,}")
        ...
        with c1:
            ...
            if not df_sales.empty:
                fig_sales = px.area(df_sales, x='month', y='revenue', title="Evoluzione Fatturato Mensile", color_discrete_sequence=['#2ecc71'])
        ...
        with c2:
            ...
            if not df_delivery.empty:
                fig_del = px.bar(df_delivery, x='customer_state', y='days', title="Tempi Consegna Medi per Stato", color='days', color_continuous_scale='Viridis')
        ...
        """, engine)
        if not df_cat.empty:
            fig_cat = px.pie(df_cat, names='product_category_name', values='orders', hole=0.4, color_discrete_sequence=px.colors.sequential.RdBu)
    ...
    st.warning("Connessione al database non inizializzata. Controlla il file .env.")
